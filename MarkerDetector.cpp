...
MarkerDetector::MarkerDetector(CameraCalibration calibration)
    : m_minContourLengthAllowed(40), markerSize(100, 100) {
  cv::Mat(3, 3, CV_32F,
  ...
  if (centerOrigin) {
    ...
    //        m_markerCorners3d.push_back(cv::Point3f(+0.5f,+0.5f,0));
    //        m_markerCorners3d.push_back(cv::Point3f(-0.5f,+0.5f,0));
    m_markerCorners3d.push_back(cv::Point3f(-6.95f, -6.95f, 0));
    m_markerCorners3d.push_back(cv::Point3f(+6.95f, -6.95f, 0));
    m_markerCorners3d.push_back(cv::Point3f(+6.95f, +6.95f, 0));
    m_markerCorners3d.push_back(cv::Point3f(-6.95f, +6.95f, 0));
  } else {
  ...
  m_markerCorners2d.push_back(
      cv::Point2f(markerSize.width - 1, markerSize.height - 1));
...
void MarkerDetector::ChangeMarkerSize(float largeSize, float smallSize,
  ...
  smallMarker3d.push_back(cv::Point3f(-smallSize, +smallSize, 0));
  frontMarker3d.clear();
  frontMarker3d.push_back(cv::Point3f(-frontSize, -frontSize, 0));
  frontMarker3d.push_back(cv::Point3f(+frontSize, -frontSize, 0));
  frontMarker3d.push_back(cv::Point3f(+frontSize, +frontSize, 0));
  frontMarker3d.push_back(cv::Point3f(-frontSize, +frontSize, 0));
}
...
void MarkerDetector::findCandidates(const ContoursVector &contours,
  ...
  for (size_t i = 0; i < possibleMarkers.size(); i++) {
    ...
    // calculate the average distance of each corner to the nearest corner of
    // the other marker candidate
...
void MarkerDetector::recognizeMarkers(const cv::Mat &grayscale,
  ...
  for (size_t i = 0; i < detectedMarkers.size(); i++) {
    ...
    // rectangular form
    cv::Mat markerTransform =
        cv::getPerspectiveTransform(marker.points, m_markerCorners2d);
...
#ifdef SHOW_DEBUG_IMAGES
    {
      ...
      cv::showAndSave("Marker " + ToString(i) + " after warp",
                      canonicalMarkerImage);
    }
#endif
    int nRotations;
    ...
    if ((id == 1) || (id == 2) || (id == 3)) {
      marker.id = id;
      // sort the points so that they are always in the same order no matter the
      // camera orientation
    ...
    //        {
    //            marker.id = id;
    //            //sort the points so that they are always in the same order no
    //            matter the camera orientation
    //            std::rotate(marker.points.begin(), marker.points.begin() + 4 -
    //            nRotations, marker.points.end());
...
#if SHOW_DEBUG_IMAGES
  {
    ...
    cv::showAndSave("Markers refined edges",
                    grayscale * 0.5 + markerCornersMat);
  }
#endif
//      cv::Mat markerCornersMat(grayscale.size(), grayscale.type());
//      markerCornersMat = cv::Scalar(0);
...
void MarkerDetector::estimatePosition(std::vector<Marker> &detectedMarkers) {
  for (size_t i = 0; i < detectedMarkers.size(); i++) {
    ...
    for (int i = 0; i < 4; i++) {
      point = m.points[i];
      xPoint = xPoint + point.x;
      yPoint = yPoint + point.y;
    }
    xPoint = xPoint / 4;
    yPoint = yPoint / 4;
    xPoints = xPoint;
    ...
    if (m.id == 1) {
      ...
      cv::solvePnP(frontMarker3d, m.points, camMatrix, distCoeff, raux, taux);
    }
    ...
    for (int col = 0; col < 3; col++) {
      for (int row = 0; row < 3; row++) {
        m.transformation.r().mat[row][col] =
